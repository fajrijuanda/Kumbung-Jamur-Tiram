  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('location', {
        header: 'Location',
        cell: ({ row }) => {
          const fullLocation = row.original.location
          const shortLocation = fullLocation.length > 25 ? fullLocation.substring(0, 25) + '...' : fullLocation

          return (
            <span title={fullLocation} className='truncate block max-w-[200px]'>
              {shortLocation}
            </span>
          )
        }
